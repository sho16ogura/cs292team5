...
func _input(event): 
	...
	if Input.is_action_just_pressed("click"): #if left mouse button is clicked
		...
		if dig_mode_state == DIG_MODES.DIG:
			var atlas_coord = Vector2i(1, 0) #riverbed tile
		...
		elif dig_mode_state == DIG_MODES.UNDIG:
...
func get_category_sur_tiles(curr_pos):
	...
	var curr_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos) #get tile data (not the whole map)
	var right_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos + Vector2i(1,0)) #get the tile at the right side
	var rightdown_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos + Vector2i(1,1)) #get the tile at the rightdown side (diagonal)
	var down_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos + Vector2i(0,1)) #get the tile at the right side
	var downleft_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos + Vector2i(-1,1)) #get the tile at the downleft side (diagnoal
	var left_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos + Vector2i(-1,0)) #get the tile at the right side
	var leftup_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos + Vector2i(-1,-1)) #get the tile at the leftup side (diagnoal)
	var up_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos + Vector2i(0,-1)) #get the tile at the right side
	var upright_tile_data : TileData = tile_map.get_cell_tile_data(ground_layor, curr_pos + Vector2i(1,-1)) #get the tile at the upright side (diagonal
